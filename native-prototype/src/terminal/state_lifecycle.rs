...
impl TerminalState {
    ...
    pub fn write_input(&mut self, text: &str) {
        ...
        // A line terminator means the shell is leaving readline and executing a
        // command.  Invalidate the authenticated prompt before the bytes reach
        // the PTY so terminal-protocol replies generated by command output
        // cannot be mistaken for fresh interactive input on Windows, where
        // termios foreground/raw-mode inspection is unavailable.
        if contains_line_terminator(text) {
