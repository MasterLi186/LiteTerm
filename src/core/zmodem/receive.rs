...
impl ZmodemReceiver {
    ...
    pub fn write_data(&mut self, data: &[u8]) -> ReceiveEvent {
    ...
    ///
    /// In a full implementation this would flush the data to disk. For now
    /// it just updates the internal counters.
    pub fn end_file(&mut self) -> ReceiveEvent {
        if let Some(file) = self.current.take() {
            self.files_received += 1;
            ReceiveEvent::FileComplete { name: file.name }
        } else {
            ReceiveEvent::Error("no file in progress".to_string())
        }
    }
